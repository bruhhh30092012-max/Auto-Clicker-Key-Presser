...
class AutoClickerPresser:
    ...
    def _fire_presses(self, send_fn, presses):
        """Same idea as _fire_clicks but for the key presser: perform
        `presses` key sends (1 = single, 2 = double, 3 = triple) with a
        small gap between them so Double/Triple press type is recognized
        as distinct presses rather than one long chord."""
    ...
    def _get_foreground_hwnd(self):
        """Return the handle of the currently focused window, or None if
        unavailable (non-Windows, or the API call failed)."""
        if not hasattr(ctypes, "windll"):
            return None
        try:
    ...
    def _get_own_hwnd(self):
        """Return the handle of this tool's own top-level window, or None
        if unavailable. tkinter's winfo_id() gives the HWND of the inner
        drawing-area/frame, which is not what GetForegroundWindow() reports
        once the window is focused - we need to walk up to the actual
        top-level owner window via GetAncestor(GA_ROOT)."""
        if not hasattr(ctypes, "windll"):
            return None
        try:
            GA_ROOT = 2
    ...
    def _get_window_title(self, hwnd):
        """Return the title text of the given window handle, or "" if
        unavailable. Switching tabs inside the same browser window keeps the
        same HWND, but the window title changes to match the active tab's
        page title - so comparing titles lets us detect a tab switch even
        when the hwnd itself doesn't change."""
    ...
    def _event_indices_for_iid(self, iid):
        """Return the sorted list of self.recorded_events indices that make
        up the (possibly grouped) row shown at `iid` in the tree, or None
        if `iid` doesn't map to a real row."""
    ...
    def _add_new_event(self):
        ...
        if category == "Key press":
            ...
            for i in range(tick_count):
                self.recorded_events.append({'type': 'wheel', 'time': t + i * WHEEL_TICK_GAP_S, 'delta': delta})
    ...
    def update_status(self):
        ...
        if self.presser_running:
            if self.presser_awaiting_target:
                running_parts.append("Presser (waiting: switch to target window)")
            elif self.presser_paused:
                running_parts.append("Presser (paused)")
            else:
        ...
        try:
            ...
            while not stop_event.is_set():
                ...
                if is_hold:
                    ...
                    self._fire_presses(lambda: _keyboard_send(key_name), presses)
    ...
    def _trim_memory(self):
        """Collect garbage, then ask Windows to trim this process's working
        set. gc.collect() only frees what Python itself no longer needs;
        SetProcessWorkingSetSize is what actually hands idle pages back to
        the OS, so Task Manager / our own RAM check reflect real usage
        instead of memory Python is quietly holding onto."""
        gc.collect()
        if hasattr(ctypes, "windll"):
            try:
                handle = ctypes.windll.kernel32.GetCurrentProcess()
