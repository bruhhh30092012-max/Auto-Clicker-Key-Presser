...
class AutoClickerPresser:
    ...
    def build_record_tab(self, record_body):
        ...
        self.record_move_interval_entry.pack(side="left", padx=(6, 6))
        ttk.Label(move_rate_row, text="(default 100ms = 10 FPS, higher = smoother GUI)",
        ...
        self.add_action_btn = tk.Button(clear_row, text="Add Action", command=self._add_new_event, width=10,
                  relief="flat", bg=COLOR_BTN_IDLE, activebackground=COLOR_BTN_IDLE_ACTIVE,
                  fg=COLOR_TEXT, bd=1, cursor="hand2")
        self.add_action_btn.pack(side="left")
        tk.Button(clear_row, text="Delete Actions", command=self._delete_selected_event, width=12,
                  relief="flat", bg=COLOR_BTN_IDLE, activebackground=COLOR_BTN_IDLE_ACTIVE,
                  fg=COLOR_TEXT, bd=1, cursor="hand2").pack(side="left", padx=(6, 0))
        tk.Button(clear_row, text="Edit", command=self._edit_selected_event, width=10,
                  relief="flat", bg=COLOR_BTN_IDLE, activebackground=COLOR_BTN_IDLE_ACTIVE,
                  fg=COLOR_TEXT, bd=1, cursor="hand2").pack(side="left", padx=(6, 0))
        tk.Button(clear_row, text="Clear", command=self.clear_recorded_events, width=10,
                  relief="flat", bg=COLOR_BTN_IDLE, activebackground=COLOR_BTN_IDLE_ACTIVE,
                  fg=COLOR_TEXT, bd=1, cursor="hand2").pack(side="left", padx=(6, 0))
        tk.Button(clear_row, text="Download", command=self._download_recorded_events, width=10,
                  relief="flat", bg=COLOR_BTN_IDLE, activebackground=COLOR_BTN_IDLE_ACTIVE,
                  fg=COLOR_TEXT, bd=1, cursor="hand2").pack(side="left", padx=(6, 0))
        tk.Button(clear_row, text="Import", command=self._import_recorded_events, width=10,
                  relief="flat", bg=COLOR_BTN_IDLE, activebackground=COLOR_BTN_IDLE_ACTIVE,
                  fg=COLOR_TEXT, bd=1, cursor="hand2").pack(side="left", padx=(6, 0))
    ...
    def _schedule_count_label_update(self):
        ...
        self.root.after(0, self._flush_count_label_update)
    ...
    def _prompt_fields(self, title, specs):
        ...
        btn_row.grid(row=len(specs), column=0, columnspan=2, pady=(6, 10))
    ...
    def _add_action_field_specs(self, category, last_t):
        ...
        if category == "Mouse move":
            return [
                {'key': 'time', 'label': 'Time (s)', 'type': 'entry', 'default': f"{last_t:.3f}"},
                {'key': 'x', 'label': 'X', 'type': 'entry', 'default': "0"},
                {'key': 'y', 'label': 'Y', 'type': 'entry', 'default': "0"},
            ]
        # Mouse wheel
        return [
            {'key': 'time', 'label': 'Time (s)', 'type': 'entry', 'default': f"{last_t:.3f}"},
    ...
    def _prompt_add_action(self, last_t):
        ...
        btn_row = tk.Frame(dialog, bg=COLOR_CARD)
        btn_row.grid(row=2, column=0, columnspan=2, pady=(6, 10))
        tk.Button(btn_row, text="Save", command=on_ok, width=8, relief="flat",
                  bg=COLOR_START, activebackground=COLOR_START_ACTIVE, fg=COLOR_TEXT, bd=1,
                  cursor="hand2").pack(side="left", padx=4)
        tk.Button(btn_row, text="Cancel", command=on_cancel, width=8, relief="flat",
        ...
        self._position_dialog_near_widget(dialog, self.add_action_btn)
        dialog.deiconify()
        dialog.grab_set()
    ...
    def _register_hotkey(self, which):
        ...
        if hot and hot != "press a key...":
            try:
                handle = keyboard.add_hotkey(hot, (lambda cmd=spec['toggle']: self.root.after(0, cmd)))
